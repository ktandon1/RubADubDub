		...
		while (true) {
			...
			if (pp.QueryRGB(image) && pp.QueryDepthMap(depthmap)) {
				...
				if((iter_ctr % iter_mod)==0) {
					...
					/*store depth pixels to file that pass all filtering
					try {
						PrintWriter writer = new PrintWriter(new FileWriter(FILE_DIR + "depth_" + im_name_ctr + ".csv", true));
						if (pp.MapDepthToColorCoordinates(p3,p2)) {
							for (int xy=0;xy<p2.length;xy++) {

								//filter out untrusted pixels
								if (depthmap[xy]==untrusted[0] || depthmap[xy]==untrusted[1]) continue;
